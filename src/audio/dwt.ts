...
export function runBpm(signal: Float32Array, Fs: number): {bpm: number, envelopeSum: Float32Array} {
  ...
  const envelopeSum = envelope1.map((v,i)=>v+envelope2[i]+envelope3[i]+envelope4[i]+envelope5[i]+envelope6[i])
...
function subBandDWT(signal: Float32Array, Fs: number, L: number, H: number): { subBand: Float32Array, numerator: number[], denominator: number[] } {
...
function envelope(SubBand: Float32Array, DecimateValue: number, Fs: number, numerator: number[], denominator: number[]): Float32Array {
...
export function AutoCorrelation(ENVELOPE: Float32Array, EnvelopeDecimated: number, MinBPM: number, MaxBPM: number): Float32Array {
